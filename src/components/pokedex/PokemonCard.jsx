    ...
    return (
        <Link to={`/pokedex/${pokemon?.id}/`} className={`text-center border-8 rounded-md shadow-lg shadow-black/40 ${bordersByType[pokemon?.types[0].type.name]}`}>
            ...
            <section className='bg-white'>
                <h3 className={`capitalize pt-16 text-3xl mb-2 font-semibold ${colorTextByType[pokemon?.types[0].type.name]}`}>{pokemon?.name}</h3>
                <h4 className='capitalize text-md text-slate-700'>{types}</h4>
                <span className='text-sm text-gray-400 mb-3'>Type</span>
                ...
                <section className='grid grid-cols-3 gap-2 p-2'>
                    {
                        pokemon?.stats.map(stat => (
                            <div className='p-4' key={stat.stat.name}>
